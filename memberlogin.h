...
int Member_login::logintomemberid()
{
	...
	cout<<"Enter passward : ";
	{
	     ...
	     {
	     	if(!strcmp(user_name,Username) && !strcmp(pass_ward,passward))
	     	{
    ...
    {
    char c;
    cout<<"\nUser name or passward incorrect ! Try again (y/n)\n";
    cin>>c;
	if(c == 'y' || c == 'Y')
	logintomemberid();
    }
...
void Member_login::logout()
{
	...
	cout<<"Enter passward of member you want to logout : ";
 		for(int i = 0;;)
 		{
 			char a = getch();
 			if((a >= 'a' && a <= 'z') || (a >= 'A' && a <= 'Z') || (a >= '0' && a <= '9'))
	...
	while(!infile.eof())
	{
		if(!strcmp(iuser,Username) && !strcmp(ipass,passward))
		{
