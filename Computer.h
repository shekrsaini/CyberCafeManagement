...
using namespace std;
 ...
 void Computer_Entry::getComputerData()
 {
	...
	cout<<"Enter Computer name : ";
 	...
 	for(int i = 0;i<strlen(ip_Adress);i++)
 	{
 		...
 		{
 			cout<<"\nip adress cannot have anything except digits and .\n";
 			goto getip;
		}
	}
	...
	if(count < 2)
	{
		cout<<"\nip adress must have atleast two \".\" \n";
		goto getip;
	}
	//ip adress chks finished
	char copyip[50];
	strcpy(copyip,ip_Adress);
 	...
 	break;
 	cin.clear();
 	cout<<"Invalid Memory ! Renter"<<endl;
 	cin.ignore(10,'\n');
    }
    int copymem;
    copymem = memory;
	...
	while(!chkcomp.eof())
	{
		...
		{
			cout<<"Unique code already exist\n ";
			goto setcode;
		}
		chkcomp.read(reinterpret_cast<char*>(this),sizeof(*this));
	}
 ...
 void Computer_Entry::showComputerData() const
 {
 	...
 	cout<<"Computer ip_Adress           : "<<ip_Adress<<endl;
