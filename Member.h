 ...
 void Member_Entry::getMemberData()
 {
	...
	cout<<"Enter name of member   : ";
 	...
 	for(int i = 0 ; i < strlen(name) ; i++)
 	{
 		...
 		if(!isalpha(name[i]))
 		{
			cout<<"\nName cannot contain anything except alphabets! Renter\n";
 			goto getname;
		}
	}
    ...
    while(!chkmem.eof())
    {
    	if(i == member_id)
    	{
    		cout<<"\nThis id alraedy exist for "<<name <<" ! Reneter\n";
    		chkmem.close();
     	    goto getid;
        }
    	chkmem.read(reinterpret_cast<char*>(this),sizeof(*this));
	...
	{
	     //cout<<"\ninvali number ! Phone number must have 11 digits\n";
	     MessageBox(NULL,"Phone number must have 11 digits","Renter!", MB_OK | MB_ICONEXCLAMATION);
		 goto getphone;
	}
	char copyph[20];
	strcpy(copyph,phone_No);
 	...
 	cout<<"Set passward      : ";
 	{
		...
		if(strlen(passward) < 5)
		{
			cout<<"\npassward should be atleast 6 digit long ! \n\t\tEnter again\n";
			goto setpass;
		}
	}
	char userchk[20];
	...
	while(!chk.eof())
	{
		if(!strcmp(userchk,Username) && !strcmp(passchk,passward))
		{
			cout<<"\nTwo members cannot have same user name and passsward\n";
			goto getuserpass;
		}
		chk.read(reinterpret_cast<char*>(this),sizeof(*this));
	}
   ...
   void Member_Entry::showMemberData() const
   {
 	...
 	cout<<"Member passward  : "<<passward<<endl;
...
remove("Tempmem.dat");
 ...
 void Member_Entry::delMemberRecord()
 {
	...
	while(!seelogin.eof())
	{
    	if(a == member_id)
    	{
		    cout<<"\nThis member is logged into your system you cannot delete this member until he is logged out\n";
	        return;
	    }
	    seelogin.read(reinterpret_cast<char *>(this),sizeof(*this));
    }
 ...
 void Member_Entry::updateMemberRecord(int a)
 {
	...
	while(!seelogin.eof())
	{
		if(a == member_id)
			{
			cout<<"\nThis member is logged into your system you cannot Update this member until he is logged out\n";
	        return;
		    }
		    seelogin.read(reinterpret_cast<char *>(this),sizeof(*this));
	}
